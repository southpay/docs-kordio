...
`POST /v1/agent/payment_intents` is a separate resource from `POST /v1/agent/actions`, but it
behaves the same way: Kordio authorizes, you execute, you report back. It holds no funds and
moves no money, so there is nothing it could execute on your behalf.

One difference remains, and it will bite you if you assume symmetry.
...
| On `allowed` | `state: pending`, you execute | `state: pending`, you execute |
| Reporting | You must call `complete` or `fail` | You must call `complete` or `fail` |

The terminal success state differs in name only: an action ends `completed`, a payment ends
`executed`. Both mean the same thing, that you told us the work went through.
