...
<Warning>
  The minor-unit amounts in the diagrams and tables below (`1000`, `2500`, ...) are written as bare numbers for readability, but on the wire every monetary value is a JSON **string** of integer minor units, for example `"1000"`. Parse money with a big-integer or decimal type, never a JS `number`. See [Core concepts](/concepts#integer-minor-units).
</Warning>
