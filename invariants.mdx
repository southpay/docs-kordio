---
title: "Invariants"
description: "The double-entry ledger invariants Kordio enforces in application code and in Postgres, and the class of money corruption each one makes impossible."
---
...
<Warning>
  The same exactness applies on the wire. Every monetary field (amounts, balances, report totals) is serialized as a JSON **string** of integer minor units (`"9700000"`, matching `^-?\d+$`), because an 18-decimal token amount can exceed `2^53` where a JSON `number` would lose precision. Parse money with a big-integer or decimal type, never a JS `number`. See [Core concepts](/concepts#integer-minor-units).
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Money flows" icon="arrow-right-arrow-left" href="/money-flows">
